...
3. **PID Controller:** Processes the error signal through three parallel branches:
   * **Proportional Path:** Multiplies the error signal by a static gain block. *(Note: In the model diagram layout, the proportional and integral gain labels are inverted—the integrator path 1/s is routed to the block labeled K_p, while the direct proportional path is routed to K_g. Functionally, these gains scale the system as K_i = 5 and K_p = 5 respectively).*
